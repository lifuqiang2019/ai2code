...
- **识别所有类型的形状**：
  ...
  * 既有填充又有边框的形状
- **填充色识别**：
  * 对于实心形状，fill.color 设置为形状的实际颜色（如 #000000 黑色）
  * 对于空心形状（只有边框），fill.color 设置为 #FFFFFF（白色/透明）
  * 如果不确定，优先识别为有填充色的形状
- **边框识别**：
  * 如果形状有明显的边框线条，hasStroke 为 true，并识别边框颜色和粗细
  * strokeWidth 范围是 3-10：3-4（细线），5-6（中等），7-10（粗线）
  * 如果形状没有明显边框，hasStroke 为 false
- **位置和大小**：
  * position 是形状中心点坐标（百分比）
  * size 是形状本身的宽度和高度（百分比）
- **其他提示**：
  * 椭圆和圆形：长宽比接近 1:1 是圆形，否则是椭圆
  * 菱形是旋转 45° 的正方形
  * 通过遮挡关系判断层级（zIndex 越大越在上层）
  * 使用标准 HEX 颜色格式（如 #FF0000）
- **重要**：务必识别图片中的所有几何形状，不要遗漏！
